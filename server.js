...
io.on("connection", (socket) => {
        ...
        userDb.auth(username, password, (cb) => {
          if (cb.err) {
            ...
            socket.emit(
              ...
              gun
                .get("mata") // Get the 'rooms' node
                .get(user.room) // Get the specific room by its name
                .get("messages")
                .map()
                .once((data) => data)
  ...
  socket.on("chatMessage", async (msg) => {
    ...
    gun.get("mata").get(user.room).get("messages").get(index).put(messageDb);

    // gun.get("mata").get(index).put(messageDb);
